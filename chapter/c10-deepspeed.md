...
## Llama-3-8B ##
이번 챕터에서는 Llama-3-8B 모델을 deepspeed 를 이용하여 훈련 한다. 
...
+--------------+---------------+------------------------+----------------------+
```
* "kubectl get nodepool" 명령어로 gpu 노드풀이 존재하는 지 확인한다. 없으면 [GPU 노드 준비](https://github.com/gnosia93/training-on-eks/blob/main/chapter/c3-gpu-node.md)를 참고하여 생성한다. 
