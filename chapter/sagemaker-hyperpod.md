...
#### 2025년 최신 기술: NCCL 복원력 (NCCL Resiliency) ####
최근에는 NCCL 자체적으로도 일부 복원력을 갖추려는 시도가 있습니다.
NCCL Config: NCCL_ASYNC_ERROR_HANDLING=1 설정을 통해 에러 발생 시 프로세스가 무한 대기하지 않고 즉시 종료되도록 하여 빠른 재시작을 돕습니다.
Torch Elastic (torchrun): 포드가 죽었을 때 쿠버네티스가 해당 포드만 다시 띄우고, NCCL 랑데부(Rendezvous)를 통해 다시 그룹을 형성하도록 돕는 기능을 사용하기도 합니다.
요약하자면: NCCL 기반 훈련은 기본적으로 "전부 아니면 전무(All or Nothing)" 방식이기에 노드 하나만 고장 나도 전체가 멈춥니다. 이 중단 시간을 최소화하고 자동으로 다시 돌려주는 것이 HyperPod의 핵심 가치입니다.
