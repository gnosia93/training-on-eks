카펜터는 스케줄링 되지 않는 파드가 클러스터 이벤트 로그에 발견되면, 이를 해결하기 위해 신규 노드를 자동으로 프로비저닝 한다. 현재 클러스터 노드 그룹은 2개로 (ng_x86 과 ng_grav) GPU 인스턴스를 스케줄링 할수 없는 그룹들이다. 파드에서 GPU 리소스를 요청하는 경우 해당 파드는 pending 상태에 빠지는데, 이떄 카페터가 나서서 GPU 노드를 클러스에 프러비저닝 해준다. 
