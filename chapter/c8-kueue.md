## 갱 스케줄러의 이해 ##
기본 쿠버네티스 스케줄러는 Pod 단위로 처리합니다. 2대의 노드가 필요한 학습에서 1대 분량의 자원만 있다면, 1개만 먼저 띄우고 나머지는 무한정 대기시킵니다. 이 경우 먼저 뜬 Pod도 학습을 시작하지 못하고 자원만 점유하는 '데드락'이 발생합니다. 이를 해결하기 위해 "모두(Gang) 아니면 무(Nothing)" 원칙으로 스케줄링해주는 엔진이 필요합니다.
보통 다음 중 하나를 설치하여 사용합니다.
