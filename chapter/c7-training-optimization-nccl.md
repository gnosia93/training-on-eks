...
### 1. EFA 환경 최적화 ###
AWS의 고속 네트워크망을 제대로 쓰려면 NCCL이 EFA를 기본 통신 계층으로 사용하도록 강제 한다.
* FI_PROVIDER="efa": 통신 프로바이더를 EFA로 지정.
* NCCL_PROTO=simple: EFA 환경에서는 복잡한 프로토콜보다 simple이 더 안정적이고 성능이 잘 나오는 경우가 많다.
* FI_EFA_USE_DEVICE_RDMA=1: GPU 간 직접 통신(RDMA)을 활성화하여 CPU 개입을 최소화.

### 2. NCCL 성능 디버깅 (로깅) ###
튜닝 전, 현재 NCCL이 어떻게 작동하는지 파악하는 것이 우선.
* NCCL_DEBUG=INFO: 모든 통신 로그를 출력한다. 로그에서 "Selected Provider is EFA" 또는 "NVLink" 사용 여부를 반드시 확인.
