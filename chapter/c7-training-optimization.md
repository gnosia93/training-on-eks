...
#### 1. 통신 병목 현상 해결 (가장 중요) ####
여러 대의 컴퓨터(노드)가 서로 데이터를 주고받는 속도가 연산 속도보다 느리면 GPU가 노는 시간이 발생한다. 
* 해결책: NVIDIA NCCL 같은 고속 통신 라이브러리 사용, 전송할 데이터를 압축(양자화)하거나, 계산이 끝나기 전에 미리 데이터를 보내는 통신-연산 중첩(Overlap) 기술을 적용한다. 
