큐브 플로우에서 제공해주는 TrainJob 오퍼레이터(트레이닝 오퍼레이터 V2)는 작업에 대한 복구 기능을 기본적으로 제공해 준다. 하드웨어 장애, 소프트웨어 설정 오류, GPU 장애, 트레이닝 코드 버그 및 NCCL 타임아웃 / 오류 등 여러가지 이유로 분산 트레이닝 잡이 실패했을때 이를 자동으로 복구해 준다. 하지만 기본 동작은 노드(파드) 중 단 하나만 실패해도 전체 노드(파드)를 다 삭제하고 새로 띄우는 'All-or-Nothing' 방식을 채택하고 있다. 분산 학습(특히 PyTorch DDP/FSDP)은 모든 노드가 엄격하게 연결된 하나의 거대한 계산기처럼 동작하기 때문이다.
이와는 별개로 Torchrun 은 노드(파드)를 재시작하지 않고 훈련을 복원할 수 있는 재시작 매커니즘을 제공하고 있다. 분산 훈련의 복원력 설정은 TrainJob 오퍼레이터의 인프라 레벨에서의 복원과 Torchrun 의 소프트웨어 레벨에서의 복원의 결합이라고 생각하면 된다.
...
```
### 2. torchrun ###
torchrun 는 각 노드에서 실행되는 일종의 데몬 프로세스로, 분산 훈련의 노드별 로컬 랭크 프로세스를 실행하고 모니터링하는 역할을 맡는다. 훈련 프로세스에 크래시가 발생(소프트웨어 버그, 설정 이슈 등) 하는 경우 이를 모니터링하여 해당 프로세스를 다시 실행 시켜준다. 
* --max_restarts: 작업이 실패했을 때 최대 몇 번까지 다시 시작할지 결정한다(기본값 0). 기본값 설정에서는 로컬 랭크 프로세스가 한 번이라도 죽으면 torchrun도 즉시 종료를 시도한다. 하지만 공유 메모리(shm)나 GPU 자원 정리가 늦어지면 상위 torchrun이 종료되지 못하고 "정리 대기 상태"로 무한 루프에 빠지며 파드가 Running으로 남게 된다.
max_restarts 의 값이 1 이면, OOM으로 죽은 프로세스를 GPU 메모리가 채 비워지기도 전에 다시 실행하려다 또 OOM이 나는 '무한 재시작 루프'에 빠질 수 있다.
* --start_method: 프로세스 시작 방식으로 보통 spawn사용하면 메모리 정리가 더 깔끔하다. spawn을 쓰면 부모 프로세스의 자원을 그대로 복사(fork)하지 않고 완전히 새로운 Python 인터프리터를 띄운다. 이전 프로세스의 남긴 찌꺼기(공유 메모리, 핸들러 등)가 다음 실행에 영향을 주지 않아 훨씬 깔끔하다. 특히 DeepSpeed처럼 복잡한 라이브러리를 쓸 때는 spawn이 안정성 측면에서 표준이다.
