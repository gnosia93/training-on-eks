...
체크포인트를 저장하는 동안 GPU 연산이 중단(Stall)되는 현상을 막는 것이다. 
또한 PyTorch Distributed Checkpoint (DCP)를 활용하여 각 Rank(프로세스)가 자신의 가중치 부분만 별도의 파일로 저장하게 함으로써 FSx와 같은 병렬 파일 시스템 아키텍처를 최대한 활용하여 쓰기 속도를 극대화하는 것이다. 
