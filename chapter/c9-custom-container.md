분산 훈련 작업에는 수많은 노드 또는 파드들이 훈련에 참여하게 되는데 작게는 수십 노드 많게는 수천 노드가 훈련에 참가 하게 된다. 
훈련 작업은 보통 관련 패키지 설치, 데이터 전처리(필요시), 그리고 실제적인 훈련 스크립트 실행으로 구성이 된다. 노드 또는 Pod 마다 처리 속도가 상황에 따라서 다르고 패키지 설치 지연 등 여러가지 요인으로 인해서 각 노드가 비슷한 시간대에 훈련 스크립트를 실행하지 못하게 되고 이로 인해 발생한 랑데뷰 타임 아웃으로 인해 훈련 작업 시작 자체가 실패하거나 지연되는 문제가 발생하게 된다.
예를 들어 플래시 어탠션 패키지는 컴파일 시간이 오래걸리는데, 노드마다 서로 상이한 설치 완료 시간으로 인해 노드간의 랑데뷰가 지연 또는 실패하는 현상이 발생 하기도 한다. 이러한 문제를 사전에 방지하기 위해서 훈련 작업에 필요한 소프트웨어 스택을 미리 구축하게 되는데, 이번 챕터에서는 이를 위한 커스텀 도커 이미지 생성 방법에 대해서 다룬다.
