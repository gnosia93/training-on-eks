...
오른쪽 상단의 CUDA Version: XX.X를 확인하세요. 런타임 이미지는 이 버전과 같거나 낮은 버전을 선택해야 안정적입니다.
...
5. Dockerfile 적용 팁
태그를 정했다면 Dockerfile 맨 윗줄에 다음과 같이 작성하세요.
dockerfile
# 2025년 1월 릴리즈된 이미지를 베이스로 사용
FROM nvcr.io/nvidia/pytorch:25.01-py3
코드를 사용할 때는 주의가 필요합니다.

주의사항: NGC 이미지는 이미 최적화된 NCCL, cuDNN, TransformerEngine 등을 포함하고 있으므로, pip install torch를 다시 실행하지 마세요. (설치된 최적화 버전이 덮어씌워져 성능이 저하될 수 있습니다.)
