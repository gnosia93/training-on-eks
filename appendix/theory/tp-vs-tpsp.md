...
## 32K Long Context ##
32K 이상의 롱 컨텍스트(Long-Context) 학습에서는 기존의 SP(Sequence Parallel)만으로는 부족하며, CP(Context Parallel) 도입이 필수적입니다. 640대의 H100 클러스터에서 32K~128K를 학습할 때의 핵심 계산과 전략을 정리해 드립니다. 1. 32K 이상에서의 병목: 어텐션(Attention) 메모리 시퀀스 길이가 \(N\)일 때, 어텐션 연산의 메모리 사용량은 \(N^{2}\)에 비례합니다. 4K → 32K: 시퀀스는 8배 늘어나지만, 어텐션 맵 메모리는 64배 증가합니다.H100 80GB 한계: SP만 사용해서는 단일 노드(8 GPU)에서도 32K 이상의 활성화 값을 담을 수 없어 OOM(Out of Memory)이 발생하거나 배치 사이즈를 1도 못 가져가는 상황이 생깁니다. 2. 해결책: Context Parallel (CP) 도입 NVIDIA Megatron-LM이나 DeepSpeed에서 제공하는 CP는 시퀀스 자체를 여러 GPU에 더 잘게 쪼개어 어텐션 연산을 분산합니다. 권장 병렬화 조합 (32K~128K 기준):TP: 8 (노드 내 NVLink 활용)CP: 2 ~ 4 (노드 간 혹은 노드 그룹 간 시퀀스 분산)SP: ON (필수)FlashAttention-3: H100 전용 커널을 사용하여 \(N^{2}\) 메모리 사용량을 획기적으로 줄여야 합니다. 3. 구체적인 수치 계산 (32K 기준) 메모리 절감: CP 4를 적용하면 32K 시퀀스를 8K씩 4개의 GPU 그룹이 나눠 가집니다. 이를 통해 개별 GPU가 부담하는 어텐션 메모리를 1/4로 감소시킵니다.통신 오버헤드: CP는 P2P(Peer-to-Peer) 통신을 빈번하게 사용합니다. H100의 400Gbps InfiniBand 환경에서는 이 통신량이 연산과 90% 이상 중첩(Overlap)되어 성능 저하를 최소화합니다.예상 Throughput:롱 컨텍스트 학습 시 MFU(연산 효율)는 일반 학습보다 낮은 35%~40% 정도로 잡는 것이 현실적입니다.초당 토큰 처리량은 시퀀스 길이에 반비례하여 감소하지만, CP 덕분에 배치 사이즈를 2 이상 유지할 수 있어 효율적인 학습이 가능해집니다. 4. 640개 H100 클러스터의 전략적 배치 항목 4K 학습 (일반)32K~128K 학습 (롱 컨텍스트)병렬화 구조TP8 + PP4 + DP20TP8 + CP4 + PP2 + DP10통신 중점NVLink 대역폭 위주InfiniBand P2P 지연시간(Latency) 위주추천 기술SP (LayerNorm 분산)CP + FlashAttention-3 + Ring Attention요약 및 제언 80노드 규모에서 32K 이상의 컨텍스트를 학습하려면, 노드 간 통신을 담당하는 인피니밴드 토폴로지(Rail-local 등)가 CP 통신을 얼마나 잘 지원하느냐가 관건입니다. 현재 구축된 클러스터의 네트워크 토폴로지(예: Fat-Tree, 1:1 Non-blocking 등) 정보를 알고 계신가요? 네트워크 구조에 따라 CP의 성능 한계치를 더 정확히 예측할 수 있습니다. 
