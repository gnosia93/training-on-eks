IPEX(Intel Extension for PyTorch)의 핵심 가속 기술인 AMX(Advanced Matrix Extensions)와 bf16 데이터 타입을 완벽하게 지원하는 AWS 인스턴스는 인텔 4세대 및 5세대 제온(Xeon) 프로세서가 탑재된 모델들입니다. AMX (Advanced Matrix Extensions): bf16과 int8 연산을 가속하는 전용 엔진입니다. IPEX에서 bf16=True를 설정하면 이 엔진을 통해 GPU와 유사한 행렬 연산 가속을 수행합니다.
Graviton(m7g 등)이나 AMD 인스턴스(m7a 등)는 인텔 전용 기술인 IPEX AMX 가속을 사용할 수 없으므로, 반드시 "i"가 붙은 인텔 인스턴스를 선택해야 합니다
