ClearML은 인프라 정보(GPU/EFA)부터 모델 실험 데이터까지 훈련의 모든 과정을 자동으로 추적·관리하고 실행 환경까지 제어할 수 있는 올인원(All-in-One) 오픈소스 MLOps 플랫폼이다.
실험 관리(Experiment Tracking)뿐만 아니라 MLOps 인프라 제어 기능을 포함하고 있어, 다른 도구보다 하드웨어 레벨의 정보를 더 깊게 자동 기록합니다. 
