...
---
결론부터 말씀드리면, 하드웨어는 준비되어 있어도 소프트웨어(OS/드라이버/컨테이너) 레벨에서 기본으로 꺼져 있거나 설정이 필요한 경우가 많습니다.
당연히 "기본"이어야 할 것 같은 P2P가 왜 기본이 아닌지, 3가지 핵심 이유를 설명해 드릴게요.
1. 보안 및 안정성 이슈 (IOMMU)
현대 운영체제는 하드웨어 장치들이 서로의 메모리에 마음대로 접근하는 것을 보안상의 이유로 차단합니다. 이를 제어하는 것이 IOMMU(Input-Output Memory Management Unit) 설정인데, 이 기능이 켜져 있으면 GPU 간의 직접 접근(P2P)이 기본적으로 막힙니다. 이를 허용하려면 리눅스 커널 파라미터에서 별도로 iommu=soft나 passthrough 설정을 해줘야 할 때가 많습니다.
2. 쿠버네티스/컨테이너 환경의 격리성
쿠버네티스 같은 컨테이너 환경은 "자원 격리"가 원칙입니다.
기본 설정: 각 컨테이너는 자신에게 할당된 GPU만 볼 수 있고, 옆에 있는 GPU가 무엇인지 모릅니다.
P2P 필요 시: NVIDIA Device Plugin 설정에서 sharing.p2p=true 같은 옵션을 명시적으로 줘야 합니다. 그래야 컨테이너가 다른 GPU의 존재를 인식하고 직접 통신할 수 있는 통로가 열립니다.
3. 하드웨어 토폴로지(구조)의 한계
모든 메인보드가 P2P를 지원하지 않습니다.
GPU 두 개가 서로 다른 CPU 소켓에 연결되어 있거나, 중간에 P2P를 지원하지 않는 PCIe 스위치가 끼어 있으면 P2P가 원천적으로 불가능합니다.
이런 경우 시스템은 안전을 위해 가장 보수적인 방법(CPU를 거치는 방식, SYS)을 기본값으로 선택합니다.
4. 드라이버의 배려 (?)
NVIDIA 드라이버는 설치 직후에 성능보다는 "안정적인 동작"에 초점을 맞춥니다. 특정 워크로드(딥러닝 등)가 아니면 P2P를 쓸 일이 없기 때문에, 애플리케이션(예: PyTorch, NCCL)이 실행될 때 "나 P2P 쓸 거야!"라고 요청해야 비로소 활성화되는 경우가 많습니다.
