...
# 2. NVIDIA CUDA 저장소 추가 (RHEL 9 계열 저장소 사용)
sudo dnf config-manager --add-repo developer.download.nvidia.com
...
* fabric manager download - https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2004/x86_64/
* 설치확인
```
nvidia-smi -q -d SUPPORTED_CLOCKS | grep -i fabric
```
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: nvidia-fabricmanager
          # 드라이버 버전(580.105.08)과 일치하는 Fabric Manager 전용 이미지 사용
          image: nvcr.io/nvidia/fabric-manager:580.105.08-ubuntu22.04
          # 이미지에 따라 실행 경로가 다를 수 있으므로 전체 경로 지정 권장
          command: ["/usr/bin/nv-fabricmanager", "-f"] 
        securityContext:
          privileged: true
        volumeMounts:
        - name: dev
