...
![](https://github.com/gnosia93/training-on-eks/blob/main/appendix/images/ic-type-speed-2.png)
* g 및 p 인스턴스들은 모두 NUMA 아키텍처(멀티 CPU 소켓)를 채용하고 있어 SYS/PHB 타입이며, PCIe 버스를 통해 GPU 간의 통신이 발생하더라도 CPU를 경유해야 하기 때문에 EFA 보다 느리다.   
* PIX (Single PCIe Switch) : 데이터가 CPU를 거치지 않고 PCIe 스위치 수준에서 바로 옆 GPU로 전달
* PXB (Multiple PCIe Switches / Hops) : 두 GPU가 서로 다른 PCIe 스위치에 연결되어 있으며, 데이터를 주고받으려면 여러 개의 PCIe 스위치를 거쳐야(Multiple Hops) 하는 구조. 
