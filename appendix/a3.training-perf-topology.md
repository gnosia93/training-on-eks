...
#### p5.48xlarge 1대 (H100) ####
A100 과 H100의 성능을 단순 비교하면 안된다. H100의 첫 실행시 쿠다 커널을 custom 하게 빌드하는 프로세스가 훨씬더 정교하고 오랫동안 동작하기 때문에 Warming UP 시간이 A100 보다 훨씬 오래 걸린다.
H100 이 재성능으로 동작하게 만들기 위해서는 튜닝 작업이 필요한 것으로 보인다.

* 1 Pod [4 GPU / 4 EFA] - 0:20:06 ( via P2P/CUMEM, P2P Type isAllDirectP2p 1 directMode 0)
* 1 Pod [4 GPU / 4 EFA] - ?? ( via ??    ) <-- PCIe 통신 ?? 
* 4 Pod [1 GPU / 1 EFA] - ?? ( via NET/Libfabric/0/GDRDMA )        
