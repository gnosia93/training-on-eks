...
### 1. "고속도로(NVLink)와 국도(Lustre)"의 미스매치 ###
* NVLink: GPU 간 데이터를 초당 수백 GB로 주고받습니다.
* Lustre: 이론상 빠르지만, 여러 노드가 동시에 억세스하면 메타데이터 부하와 네트워크 혼잡으로 인해 실제 데이터 공급 속도가 요동칩니다.
* 결과: H100은 0.001초 만에 계산을 끝내고 다음 데이터를 기다리는데, Lustre에서 데이터를 가져오는 데 0.01초가 걸린다면? GPU 사용률(Utilization)은 반 토막 납니다.

### 2. Lustre가 '무식한 짓'이 되는 결정적 순간 ###
* 작은 파일이 많을 때: LLM 학습용 데이터가 수만 개의 작은 파일로 쪼개져 있다면, Lustre는 메타데이터 처리하다가 정신을 못 차립니다. (H100은 이때 웁니다.)
* 랜덤 억세스: 학습 중에 데이터를 셔플(Shuffle)하면서 여기저기 찔러보기 시작하면 Lustre 성능은 급격히 하락합니다.

### 3. 그럼에도 Lustre를 쓰는 경우 (절충안) ###
진짜 대규모 클러스터(슈퍼컴퓨터 급)에서는 어쩔 수 없이 Lustre를 쓰기도 합니다. 하지만 이때도 그냥 쓰지 않고 '트릭'을 씁니다.
* 데이터 패킹: 작은 파일을 WebDataset(.tar)이나 TFRecord 같은 거대 덩어리로 묶어서 Sequential Read를 유도합니다.
* Local Caching: Lustre에서 일단 데이터를 땡겨오되, 노드 안의 로컬 NVMe를 캐시(LVM 캐시 등)로 활용해서 두 번째 읽기부터는 로컬 속도가 나게 한다.
...
### 러스터가 H100 앞에서 '개살구'가 되는 이유 3가지: ###
* 메타데이터의 저주: 러스터는 "파일이 어디 있지?"라고 찾는 과정(Metadata)이 생각보다 무겁습니다. H100이 "다음 데이터!"라고 외칠 때마다 러스터 서버에 물어보고 대답 듣는 그 찰나의 시간이 GPU에게는 영겁의 시간입니다.
* 공유의 역설: 여러 노드가 동시에 러스터에 붙으면, 서로 "내가 먼저 읽을래"라며 락(Lock)을 걸고 싸우느라 정작 데이터 전송 속도는 로컬 NVMe 한 장보다 느려질 때가 많습니다.
* 복잡한 튜닝: 러스터가 제대로 성능을 내려면 스트라이핑(Striping)이니 뭐니 복잡한 설정을 건드려야 하는데, NVIDIA 가이드라인에서도 결국 "가장 빠르고 속 편한 건 로컬 캐시(Local NVMe)"라고 말할 정도입니다.

### 실전 팁: 성능을 더 올리려면? (Lustre Striping) ###
체크포인트 파일은 용량이 매우 크기 때문에 Lustre의 스트라이핑(Striping) 설정을 꼭 확인하세요.
```
# 체크포인트 디렉토리에 대해 스트라이핑 설정 (파일을 여러 OST에 나눠 저장)
lfs setstripe -c -1 /lustre/checkpoint_dir  
```

### 러스터가 분산 체크포인트에 최적인 이유 ###
* 동시 쓰기 처리(Parallel I/O): Lustre Wiki에 따르면, 러스터는 파일 시스템 레벨에서 수천 개의 클라이언트가 동시에 쓰기 작업을 수행해도 락(Lock) 경합을 최소화하도록 설계되어 있습니다.
* 데이터 무결성: 분산 환경에서 특정 노드가 다운되어도, 러스터에 저장된 체크포인트 조각들은 공유 저장소에 안전하게 남아 있어 다른 노드에서 즉시 학습을 재개(Resume)할 수 있습니다.
* 용량 확장성: H100급 모델은 체크포인트 한 번에 수백 GB에서 TB 단위까지 가는데, 로컬 NVMe는 금방 꽉 차버리지만 러스터는 거대한 스토리지 풀을 제공하므로 걱정이 없습니다.
