...
PyTorch 분산 학습 모델(DDP)은 로컬(1대)이나 멀티 노드(여러 대)나 동일한 코드로 돌아가도록 설계되어 있다. 따라서 로컬에서도 자기 자신에게 데이터를 보내는 Loopback 인터페이스(127.0.0.1)를 통해 네트워크 패킷을 주고 받는다. gloo 백엔드 역시 기본적으로 TCP 소켓 통신을 기반으로 하는데 각 랭크(프로세스)는 특정 포트를 열고 대기하며, 다른 랭크들과 데이터를 교환한다.
